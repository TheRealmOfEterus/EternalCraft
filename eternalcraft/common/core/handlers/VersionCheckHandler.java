...
public class VersionCheckHandler {
	...
	public static final void checkVersion() {
		...
		result = NOT_DONE;
		try {
			final URL remoteVersionURL = new URL(remoteFileLocation);
			remoteVersionStream = remoteVersionURL.openStream();
			remoteVersionProperties.loadFromXML(remoteVersionStream);
			final String versionFromRemote = remoteVersionProperties.getProperty(Loader.instance().getMCVersionString());
	...
	public static final void checkLatestChanges() {
		InputStream remoteChangesStream = null;
		try {
			final URL remoteChangesURL = new URL(remoteChangesLocation);
			remoteChangesStream = remoteChangesURL.openStream();
			remoteChangesProperties.loadFromXML(remoteChangesStream);
			final String changesFromRemote = remoteChangesProperties.getProperty(Reference.LATEST_VERSION);
	...
	public static final class VersionChecker implements Runnable {
