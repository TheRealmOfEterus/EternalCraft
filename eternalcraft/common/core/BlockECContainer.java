...
/**
 * A base for most of the blocks that will have an inventory of some sort.
 * Handles directional placing, dropping inventory, etc. All children will
 * inherit this behavior.
 * 
 * @author bau5
 */
public abstract class BlockECContainer extends BlockContainer {
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase living, ItemStack stack) {
		...
		byte dir = 0;
		final int plyrFacing = MathHelper.floor_double(((living.rotationYaw * 4F) / 360F) + 0.5D) & 3;
		if (plyrFacing == 0) {
			dir = 2;
		}
		if (plyrFacing == 1) {
	...
	private final void dropItems(World world, int x, int y, int z) {
		...
		for (; i < size; i++) {
			...
			if (item != null && item.stackSize > 0) {
				...
				final EntityItem ei = new EntityItem(world, x + rx, y + ry, z + rz, new ItemStack(item.itemID, item.stackSize, item.getItemDamage()));
